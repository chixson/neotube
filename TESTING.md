...
- **Ranged sampling**: after a fit, use `python -m neotube.replica_cli --posterior runs/ceres/fit_no_kepler/posterior.npz --ranged --n 2000 --n-proposals 50000 --n-workers 8 --scoring-mode kepler --top-k-nbody 2000 --output runs/ceres/replicas_ranged_2k.csv` (add `--no-kepler` to force full n-body scoring).
